...
$sql = "SELECT COUNT(*) AS cnt FROM yy_wiki_board WHERE `category` = '양디위키'";
$data = fetch($con, $sql);
...
$cat_yywiki_sql = "SELECT `id`, `category`, `category_src`, `title`, `ip` FROM `yy_wiki_board` WHERE `category` = '양디위키'  ORDER BY `id` DESC Limit " . ($page - 1) * 25 . ", 25";
